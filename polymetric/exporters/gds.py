...
def save(shape, filename, cell_name="POLYGON", datatype=1000, ignore_interiors=False, **kw):
    ...
    if not ignore_interiors and shape.has_interiors():
        raise ValueError(
            "Polygon contains interiors, which can not be represented in the GDS file format. To ignore, call with ignore_interiors=True")

    vertex_lists = shape.get_exterior_vertex_lists()

    for vl in vertex_lists:
        if len(vl) > MAX_VERTICES:
            raise ValueError("Polygon contains %d vertices, more than the limit of %d vertices" % (
                len(vl), MAX_VERTICES))
